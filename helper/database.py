...
class Database:
    ...
    def new_user(self, id):
        return dict(
            ...
            caption=None,
            metadata=True,
            metadata_code="Telegram : @CrunchyRollChannel",
    ...
    async def get_title(self, user_id):
        user = await self.col.find_one({'_id': int(user_id)})
        return user.get('title', 'Encoded by @CrunchyRollChannel')
    ...
    async def get_author(self, user_id):
        user = await self.col.find_one({'_id': int(user_id)})
        return user.get('author', '@CrunchyRollChannel')
    ...
    async def get_artist(self, user_id):
        user = await self.col.find_one({'_id': int(user_id)})
        return user.get('artist', '@CrunchyRollChannel')
    ...
    async def get_audio(self, user_id):
        user = await self.col.find_one({'_id': int(user_id)})
        return user.get('audio', 'By @CrunchyRollChannel')
    ...
    async def get_subtitle(self, user_id):
        user = await self.col.find_one({'_id': int(user_id)})
        return user.get('subtitle', "By @CrunchyRollChannel")
    ...
    async def get_video(self, user_id):
        user = await self.col.find_one({'_id': int(user_id)})
        return user.get('video', 'Encoded By @CrunchyRollChannel')
